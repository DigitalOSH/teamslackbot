...
def process_message(data):
     if data['text'].startswith("!uidscore"):
        info = getdbconfig()
        db = pymysql.connect(info['host'], info['user'], info['passwd'], info['dbname'])
        cursor = db.cursor()
        score = data['text'].replace("!uidscore","")
        score = score.replace(" ","")
        print(score)
        try:
            gameuid = score.split("|")
            ...
            print(e)
            outputs.append([data['channel'], "Sorry, that didn't work. Correct usage: !uidscore <Our Score>-<Their Score>|<UID>"])
        if score == "":
            outputs.append([data['channel'], "Sorry, that didn't work. Correct usage: !uidscore <Our Score>-<Their Score>|<UID>"])
        else:
            try:
                sql = "SELECT * FROM games WHERE uid = '%s'" % (gameuid)
                cursor.execute(sql)
